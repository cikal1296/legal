  ...
  <path d="M16 46c2.5-7 9-11 16-11s13.5 4 16 11" fill="none" stroke="#2563EB" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
            ...
            <div class="flex flex-col">
                ...
                <div class="flex items-center gap-2 2xl:gap-4">
                    ...
                    <span
                        class="px-2 2xl:px-2 py-0.5 2xl:py-1 rounded-full bg-blue-600 text-white text-xs 2xl:text-sm font-medium">
                        Client
                    </span>
                </div>
                ...
                <div class="flex gap-4 2xl:gap-6 mt-1 2xl:mt-3 text-sm 2xl:text-base">
                    <button
                        :class="activeTab === 'all' ? 'text-blue-600 font-medium border-b-2 border-blue-600 pb-1 2xl:pb-3' :
                            'text-gray-500 hover:text-black'"
                        @click="activeTab='all'">Semua</button>

                    <button
                        :class="activeTab === 'komunikasi' ?
                            'text-blue-600 font-medium border-b-2 border-blue-600 pb-1 2xl:pb-3' :
                            'text-gray-500 hover:text-black'"
                        @click="activeTab='komunikasi'">Komunikasi</button>

                    <button
                        :class="activeTab === 'tagihan' ? 'text-blue-600 font-medium border-b-2 border-blue-600 pb-1 2xl:pb-3' :
                            'text-gray-500 hover:text-black'"
                        @click="activeTab='tagihan'">Tagihan</button>

                    <button
                        :class="activeTab === 'transaksi' ?
                            'text-blue-600 font-medium border-b-2 border-blue-600 pb-1 2xl:pb-3' :
                            'text-gray-500 hover:text-black'"
                        @click="activeTab='transaksi'">Transaksi</button>

                    <button
                        :class="activeTab === 'dokumen' ? 'text-blue-600 font-medium border-b-2 border-blue-600 pb-1 2xl:pb-3' :
                            'text-gray-500 hover:text-black'"
            ...
            <div class="xl:col-span-1">
                <div class="bg-white border rounded-xl p-4 2xl:p-6 shadow-sm space-y-3 2xl:space-y-5">
                    ...
                    <div class="mt-3 2xl:mt-5">
                        ...
                        <textarea rows="3" class="w-full border h-40 rounded-lg mt-1 2xl:mt-3" x-model="note"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="xl:col-span-3 space-y-6 2xl:space-y-8">
                <div class="bg-white border rounded-xl p-4 2xl:p-6 shadow-sm">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-3 2xl:gap-5">
                            ...
                            <button @click="detailOpen = !detailOpen" class="text-gray-600 hover:text-black">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    :class="detailOpen ? 'transform rotate-180' : ''" class="w-4 2xl:w-6 h-4 2xl:h-6"
                        ...
                        <div class="flex items-center gap-3">
                            <button @click="showAddJob=true"
                                class="px-3 2xl:px-5 py-1.5 2xl:py-3 bg-blue-600 text-white text-sm 2xl:text-base rounded-lg hover:bg-blue-700">
                                Tambah Pekerjaan
                            </button>
                    ...
                    <div class="mt-4 2xl:mt-6">
                        <span
                            class="px-3 2xl:px-3 py-1 2xl:py-1 bg-green-100 text-green-700 rounded-full text-xs 2xl:text-sm font-medium">
                            Status: Open
                        </span>
                    </div>

                    <!-- Info Row -->
                    <div x-show="detailOpen" class="grid grid-cols-1 md:grid-cols-3 gap-4 2xl:gap-4 mt-4 2xl:mt-6"
                        x-transition>
                        <div class="p-3 2xl:p-5 bg-gray-50 rounded-lg">
                            <p class="text-gray-500 text-sm 2xl:text-base">Sisa tagihan</p>
                            <p class="font-semibold text-lg 2xl:text-xl">IDR <span
                                    x-text="formatNumber(remaining)"></span></p>
                        </div>

                        <div class="p-3 2xl:p-5 bg-gray-50 rounded-lg">
                            <p class="text-gray-500 text-sm 2xl:text-base">Dana amanat klien</p>
                            <p class="font-semibold text-lg 2xl:text-xl">IDR <span x-text="formatNumber(escrow)"></span>
                            </p>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 2xl:gap-8">
                    <div class="bg-white border rounded-xl p-4 2xl:p-6 shadow-sm">
                        ...
                        <p class="text-sm 2xl:text-base"><span class="font-medium">Pajak yang harus dibayar:</span> IDR
                            5.000.000</p>
                    </div>

                    <div class="bg-white border rounded-xl p-4 2xl:p-6 shadow-sm">
                        <h2 class="font-semibold text-gray-700 mb-2 2xl:mb-3">Informasi Pembayaran</h2>
                        <p class="text-sm 2xl:text-base"><span class="font-medium">No. Rekening BCA:</span> 013xxxxxxx
                        </p>
                        <p class="text-sm 2xl:text-base"><span class="font-medium">No. Rekening BNI:</span> 00xxxxxxx
                        </p>
                    </div>
                </div>
                                    ...
                                    <div class="text-right">
                                        <template x-if="item.type==='tagihan' || item.type==='transaksi'">
                                            <p class="font-semibold"
                                                x-text="item.amount ? 'IDR ' + formatNumber(item.amount) : ''"></p>
                                        </template>
                                        ...
                                        <template x-if="item.type==='dokumen'">
                                            <a :href="item.fileUrl || '#'"
                                                class="text-sm text-blue-600 hover:underline">Download</a>
                                        </template>

                                        <div class="mt-2 flex items-center gap-2">
                                            <button @click="editItem(item)"
                                                class="px-2 py-1 border rounded text-sm">Edit</button>
                                            <button @click="deleteItem(item.id)"
                                                class="px-2 py-1 border rounded text-sm text-red-600">Hapus</button>
                                        </div>
                                    </div>
                                </div>
                            </template>
        ...
        <div x-show="showAddJob" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4"
            x-transition>
            <div class="bg-white rounded-xl p-6 w-full max-w-md space-y-4">
                <h2 class="text-lg font-semibold">Tambah Pekerjaan / Item</h2>

                <input type="text" x-model="newItem.title" placeholder="Judul"
                    class="w-full border rounded-lg p-2">
                <input type="text" x-model="newItem.subtitle" placeholder="Deskripsi singkat"
                    class="w-full border rounded-lg p-2">
                ...
                <div class="flex gap-2">
                    ...
                    <input x-show="newItem.type==='tagihan' || newItem.type==='transaksi'" type="number"
                        x-model.number="newItem.amount" placeholder="Jumlah (IDR)"
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button @click="addItem()"
                        class="px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Tambah</button>
                </div>
            </div>
        </div>

        <!-- ====================== MODAL TAMBAH PENGELUARAN ====================== -->
        <div x-show="showExpense" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4"
            x-transition>
            <div class="bg-white rounded-xl p-6 w-full max-w-md space-y-4">
                <h2 class="text-lg font-semibold">Tambah Pengeluaran (Transaksi)</h2>

                <input type="text" x-model="expenseForm.title" placeholder="Nama pengeluaran"
                    class="w-full border rounded-lg p-2">
                <input type="number" x-model.number="expenseForm.amount" placeholder="Jumlah (IDR)"
                    class="w-full border rounded-lg p-2">
            ...
            return {
                ...
                items: [
                    // komunikasi
                    {
                        id: 1,
                        type: 'komunikasi',
                        title: 'Chat: Tanya progres',
                        subtitle: 'Pesan singkat dari klien',
                        date: '2025-11-10'
                    },
                    ...
                    // tagihan
                    {
                        ...
                        type: 'tagihan',
                        title: 'Invoice #1001',
                        subtitle: 'Pembayaran bulan Sept',
                        amount: 5000000,
                        date: '2025-10-01'
                    },
                ...
                totalExpensesCalc() {
                    const total = this.items.filter(i => i.type === 'transaksi').reduce((s, it) => s + (it.amount ? Number(
                        it.amount) : 0), 0);
                ...
                init() {
                    ...
                    this.totalExpenses = this.items.filter(i => i.type === 'transaksi').reduce((s, it) => s + (it.amount ?
